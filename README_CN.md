...
**位姿跳变校正 —— 默认关闭，实测有害。**
`build_map.py` 可以检测单采样不连续（`|dp|/dt` 超过 `v_max`，默认 1.0 m/s）
并将其后的所有位姿加上偏移。但用轮式里程计验证后发现这样做**让轨迹更差**：
那些「回跳」是估计器在纠正一段跑超了的轨迹，把偏移再加回去等于把误差重新注入。
仅在处理其他数据集时用 `BUILD_MAP_JUMP_FIX=1` 开启。

### VIO 到底有多准？

用 bag 中轮子关节（`base_link → wheel_left/right_link`，半径 0.033m，轮距 0.287m）
积分出的轮式里程计做验证：

| 区间 | VIO vs 轮式 |
|------|-------------|
| t = 252–873s | 分段尺度 **0.977 / 1.046 / 1.022 / 1.020 / 1.009** |
| t = 850–955s | 局部对齐尺度 **0.9950**，RMSE **0.020 m** |
| t ≈ 955–975s | 误差从 0.07m 涨到 **3.55m**；真值显示机器人运动很慢（0.11 m/s、航向恒定），VIO 却报告了约 2 倍的运动量 |
| t ≈ 1121–1245s | 尺度 1.12 |

也就是说：**VIO 在绝大部分时间近乎精确，建图分离来自两个局部时段**，而非全局尺度误差。
整段 bag 中 VIO 走了 90.7m 路径，而轮子只转了 79.4m。

> 注意：轮式里程计本身也是积分，会缓慢漂移；在上述窗口内它是很好的相对参考，
> 但不是绝对真值。
...
Noetic 的默认安装链接了 OctoMap，因此 `Grid/3D` 默认为 **true**——2D 栅格是从八叉树
投影出来的，**2D 光线投射不会生效**。关掉光线投射后栅格只做命中计数：
一个错误的深度点会永久占据该格，这正是放射状条纹的成因。

| 参数 | 默认 | 本项目 | 原因 |
|------|------|--------|------|
| `Grid/3D` | true | **false** | 走 2D 路径才能启用光线投射，且更省算力 |
| `Grid/RayTracing` | false | **true** | 雕刻自由空间，擦除瞬态错误点 |
| `Grid/RangeMax` | 5.0 | **4.5** | bag 实测的传感器截断距离；20 会引入远处离群点 |
| `Grid/RangeMin` | 0 | **0.2** | 丢弃近处飞点 |
| `Grid/DepthRoiRatios` | 0 0 0 0 | **0 0 0 0.26** | 每帧第 355–479 行 100% 无效 |
| `Grid/NoiseFilteringRadius` | 0.0 | **0.1** | 离群点滤除（原为关闭） |
| `Grid/NoiseFilteringMinNeighbors` | 5 | 5 | 保持 |
| `Grid/MaxGroundHeight` | 0.0 | **0.20** | 机器人高度的地面分割 |

**深度流的实测事实**（调参前值得知道）：有效比例恒为 **62.5% = 恰好 300/480 行**——
每帧底部 26% 完全无效，最大量程被截断在 4.49m。这是**结构性问题，不是传感器噪声**，
任何形态学填充都补不上。

**`house_full.bag` 400 秒回放 A/B 实测**（输入完全相同，只有栅格参数不同）：

| | 地图尺寸 | 障碍格 | 自由格 |
|---|---|---|---|
| 默认参数 | 264×227 | 15,035 | 6,318 |
| 上表参数 | 236×215 | 4,577 | 22,855 |

默认参数的地图布满放射状拖影和噪点；新参数下条纹消失、自由空间被正确雕刻。
圆形边界只是 4.5m 射线量程的体现。

> **但「干净」不等于「结构完整」。** 对照人工标注的户型图
> （`maps/ground_truth_annotated.png`，并排对比见 `docs/map_quality_comparison.png`）：
>
> | 版本 | 房屋结构 |
> |------|----------|
> | 人工标注（真值） | 完整：外墙、隔断、家具块、门洞 |
> | **离线 `build_map.py`** | **最接近真值**——房间划分与家具清晰可辨 |
> | RTAB-Map 默认参数 | 一坨不可辨认的黑斑 |
> | RTAB-Map 新参数 | 条纹消失，**但墙体被打碎、结构残缺** |
>
> 光线投射消除了条纹，但也侵蚀了薄墙和低矮家具。要一张能用的地图，
> 当前推荐：**离线 `build_map.py` 出图，RTAB-Map 只负责实时定位（`map→odom`）**。
> 实时建图要达到离线质量，还需更细的调参或更换建图前端。
